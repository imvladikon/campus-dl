...
"""
Main module for the campus-dl downloader.
...
OPENEDX_SITES = {
    'campus': {
        'url': 'https://courses.campus.gov.il',
        'courseware-selector': ('nav', {'aria-label': 'Course Navigation'}),
    },
    'edge': {
        'url': 'https://edge.campus.gov.il',
        'courseware-selector': ('nav', {'aria-label': 'Course Navigation'}),
    },
    'stanford': {
...
BASE_URL = OPENEDX_SITES['campus']['url']
CAMPUS_HOMEPAGE = BASE_URL + '/user_api/v1/account/login_session'
LOGIN_API = BASE_URL + '/login_ajax'
DASHBOARD = BASE_URL + '/dashboard'
COURSEWARE_SEL = OPENEDX_SITES['campus']['courseware-selector']
...
def campus_get_subtitle(url, headers,
...
def campus_login(url, headers, username, password):
    """
    Log in user into the campus website.
    """
    logging.info('Logging into Open campus site: %s', url)
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(prog='campus-dl',
                                     description='Get videos from the campus platform',
    ...
    parser.add_argument('course_urls',
                        ...
                        '(e.g., https://courses.campus.org/courses/BerkeleyX/CS191x/2013_Spring/info)')
    ...
    parser.add_argument('-u',
                        ...
                        required=True,
                        action='store',
                        help='your campus username (email)')

    parser.add_argument('-p',
                        '--password',
                        action='store',
                        help='your campus password, '
    ...
    parser.add_argument('-x',
                        ...
                        action='store',
                        dest='platform',
                        help='campus platform, one of: %s' % ', '.join(sites),
                        default='campus')
...
def campus_get_headers():
    ...
    headers = {
        'User-Agent': 'campus-downloader/0.01',
        'Accept': 'application/json, text/javascript, */*; q=0.01',
        'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8',
        'Referer': CAMPUS_HOMEPAGE,
        'X-Requested-With': 'XMLHttpRequest',
        'X-CSRFToken': _get_initial_token(CAMPUS_HOMEPAGE),
    }
...
def get_subtitles_urls(available_subs_url, sub_template_url, headers):
    ...
    """
    sub_template_url_fix = sub_template_url.replace("transcript/translation/%s", "transcript/download?lang=%s")
    if available_subs_url is not None and sub_template_url_fix is not None:
        try:
        ...
        return {sub_lang: sub_template_url_fix % sub_lang
                for sub_lang in available_subs}

    elif available_subs_url is not None and sub_template_url is not None:
        try:
            available_subs = get_page_contents_as_json(available_subs_url,
                                                       headers)
        except HTTPError:
...
def download_subtitle(url, filename, headers, args):
    ...
    """
    subs_string = campus_get_subtitle(url, headers)
    if subs_string:
...
def main():
    ...
    """
    args = parse_args()
    logging.info('campus_dl version %s', __version__)
    ...
    # Prepare Headers
    headers = campus_get_headers()

    # Login
    resp = campus_login(LOGIN_API, headers, args.username, args.password)
